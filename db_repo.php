		...
		try {
			if ($param[docFileNumber] == null && $param[docFileNumber] == "") {
				...
				if ($param[filter][fileNumber] != null) {
					...
					if ($param[filter][status] != null) {
						if ($param[filter][status] == 12)
							$where .= " AND (doc.status = 1 OR doc.status = 2)";
						else
							$where .= " AND doc.status = '{$param[filter][status]}'";
					}
				}
					
				$st = "SELECT docFileNumber, docDate, docApprover, docArea, docBlock, docStreet, docBuilding, docPACINumber, docTitle, docComment FROM doc";
				...
				$st = "SELECT docFileNumber, docDate, docApprover, docArea, docBlock, docStreet, docBuilding, docPACINumber, docTitle, docComment FROM doc WHERE docFileNumber = :docFileNumber";
...
//				continue;
			...
			//if ($param == "") {
				...
				if($stDoc->fetchColumn() == 0)
				{
					$st = "INSERT INTO doc (docFileNumber, docDate, docApprover, docArea, docBlock, docStreet, docBuilding, docPACINumber, docTitle)
						...
						ON DUPLICATE KEY UPDATE docDate=:docDate, docApprover=:docApprover, 
	...
	public function approve_reject($param) {
		...
		try {
			...
			//if ($param[status])
			$st .= ", status = " . $param[status];

			$st .= " WHERE docFileNumber = $param[docFileNumber]";
				
			//$st = "UPDATE doc SET docComment = " . $dbh->quote($param[docComment]) . " WHERE docFileNumber = $param[docFileNumber]";
