...
*/
	...
	if (actor == actor_enum.employee && sectionId == sectionId_enum.followup) {
		...
		if (actor == actor_enum.manager) {
			...
			} else if (actor == actor_enum.manager && sectionId == sectionId_enum.followup && $("#tabs").tabs( "option", "active" ) == 0) {
				itemTemplate.find("a:nth-of-type(1)").addClass("rejectButton");				//reject button
...
*/
	...
	var html = '<div style="overflow:auto; height: 340px; word-wrap:break-word;">' + docComment + "</div><br/>";
			...
			if (action == status_enum.readonly) {
				...
				$("#dialog-form-comments" ).dialog( "option", "buttons",
					[{	text: "Ok",
						click: function() {
							if (action == status_enum.approve)
								approve(this.docComment, this.docFileNumber);
							else if (action == status_enum.reject)
...
function approve(docComment, docFileNumber) {
	approve_reject(docComment, docFileNumber, true)
}

function reject(docComment, docFileNumber) {
	approve_reject(docComment, docFileNumber, false)
}

function approve_reject(docComment, docFileNumber, approve) {
	var currentComment;
